...
$LOAD_PATH << File.join(File.dirname(__FILE__, 2), "src")

ODDB_URL = ENV["ODDB_URL"] || "http://127.0.0.1:8012"
...
PROJECT_ROOT = testing_ch_oddb_org ? "/var/www/oddb.org" : File.dirname(__FILE__, 2)
...
def setup_browser
  ...
  if Browser2test[0].to_s.eql?("firefox")
    ...
    Selenium::WebDriver::Firefox.path = if browser_path.length > 0
      browser_path.chomp
    else
      "/usr/bin/firefox"
    end
    ...
    Selenium::WebDriver::Chrome.path = if browser_path.length > 0
      browser_path.chomp
    else
...
def createScreenshot(browser, added = nil)
  return
  name = if browser.url.index("?")
    File.join(ImageDest, File.basename(browser.url.split("?")[0]).gsub(/\W/, "_"))
  else
    File.join(ImageDest, browser.url.split("/")[-1].gsub(/\W/, "_"))
  end
  name = "#{name}#{added}.png"
...
def check_download(element_to_click)
  ...
  filesAfterDownload - filesBeforeDownload
end
...
def select_product_by_component(name)
  begin
    @browser.goto ODDB_URL
  ...
  begin
    if @browser.element(id: "ikscat_1").wait_until(&:present?)
