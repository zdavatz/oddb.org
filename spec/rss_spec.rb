...
describe "ch.oddb.org" do
  ...
  def check_for_rss_feed(link = nil)
    ...
    if link
      if @browser.name.eql?(:chrome)
        skip("Chromium takes way to long to open an RSS feed") unless /recall/.match?(link)
        @browser.goto(link)
  ...
  tests.each { |subject, test_string|
    it "should have a working RSS-feed /de/gcc/rss/channel/#{subject}" do
      ...
      /fachinfo/i.match?(subject) || false
