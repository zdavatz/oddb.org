...
describe ParseComposition do
  ...
  xit "should handle isknr 65499" do
    string = "netupitantum 300 mg, palonosetronum 0.5 mg ut palonosetroni hydrochloridum, cellulosum microcristallinum, sacchari lauras corresp. saccharum 20 mg, povidonum K 30, carmellosum natricum conexum, silica colloidalis hydrica, natrii stearylis fumaras, magnesii stearas, macrogol 6 glyceroli cadebuglocapras, glycerolum, polyglyceroli-3 mono-oleas, aqua purificata, E 320, matériel de la capsule: gelatina, sorbitolum 7 mg, 1,4-sorbitanum, E 171, E 172 (flavum), E 172 (rubrum), encre: lacca, E 172 (nigrum), propylenglycolum, ammonii hydroxidum pro capsula corresp. natrium 0.38 mg."
    composition = ParseComposition.from_string(string)
    expect(composition.source).to eq string
    expect(composition.substances.size).to be > 1
  end
  ...
  xit "should handle isknr 66778" do
    string = "Beutel: tisagenlecleucelum CAR-positive lebensfähige T-Zellen. Enthält genetisch veränderte Zellen 1,2x10e6–6,0x10e8 pro dosi, natrium, chloridum, magnesium, acetas, gluconas, glucosum, 5-hydroxymethylfurfuralum, albuminum, N-acetyltryptophanum natricum, cadebuglas, aluminium, dextranum 40, dimethylis sulfoxidum, dimethylis sulfonum, q.s. pro 50 ml."
    composition = ParseComposition.from_string(string)
    expect(composition.source).to eq string
    expect(composition.substances.size).to be > 1
  end
