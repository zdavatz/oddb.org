...
- **Restoring is a different question from stopping.** The fix loses nothing further; it recovers nothing either. Of ~7400 `Patinfo` objects with no document at all, only **524 combinations of registration and language over 273 drugs** have an orphaned diff whose `@string2` still holds the full text of the day. For the rest the text is gone, not merely unreachable.
