...
- Verify by measuring, not by looking: headless Chromium, `document.body.scrollWidth` against `clientWidth`, and a contrast walk over every leaf element. Snap Chromium cannot write to `/tmp` and hangs on `file://` — write screenshots under `$HOME` and serve pages over `127.0.0.1`.
...
- **Keep the XML.** Feedly polls `recall`, `sl_introduction`, `price_cut`, `price_rise` and `hpc` about 57 times a day each (~290 total). `fachinfo.rss` gets 2. The cost is all on the fachinfo side: `update_fachinfo_rss_feeds` reports **1226 seconds** for the yearly feeds, peaks at 3.9 GB, and `data/rss/` holds 2.1 GB.
