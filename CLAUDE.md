...
- **The verifier (4.x) accepts only DCQL.** Sending `presentation_definition` fails with `{"error_description":"dcqlQuery: must not be null"}`. The request body therefore uses `dcql_query`; the `CREDENTIAL_FORMAT` constant selects the media type (`vc+sd-jwt`, matching what the issuer emits — `dc+sd-jwt` is the newer canonical name and also accepted).
