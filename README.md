...
`jobs/update_drugshortage` imports current Swiss drug shortages from [drugshortage.ch](https://www.drugshortage.ch). In May 2026 the upstream site migrated from an ASP.NET HTML page to a WordPress + JSON API; the plugin fetches `https://www.drugshortage.ch/api_engpaesse.php` (the official "show all current shortages" endpoint, ~713 active records) and parses `gtin`, `mutation`, `status`, `lieferdatum`, `id` per record. `shortage_link` is built from the stable upstream `id` as `https://www.drugshortage.ch/index.php/detail-lieferengpass/?ID=<id>`. Cached fixture lives at `data/json/drugshortage-latest.json`.
