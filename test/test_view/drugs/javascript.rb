...
puts "#{__FILE__} ENV['TRAVIS'] is #{ENV['TRAVIS'].inspect}"
if ENV['TRAVIS']
  puts "Skipping javascript test on TRAVIS"
else
  ...
  class TestJavaScript <Minitest::Test
    ...
    def test_prescription
      ...
      assert(File.directory?(javascripts))
      ...
      FileUtils.makedirs(File.dirname(outfile), :verbose => true)
      assert(File.directory?(File.dirname(outfile)))
      script = File.join(javascripts, 'prescription.js')
      ...
      cmd = "/bin/cat #{script} #{javascripts}/qrcode.js #{__FILE__.sub('.rb','.js')} | /usr/bin/tee #{outfile} | /usr/bin/nodejs"
      result = `#{cmd}`.chomp
