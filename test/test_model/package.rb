...
class TestPackage < Minitest::Test
  ...
  def create_test_package(iksnr:, ikscd:, price_public:, ddd_dose:, atc_code:,
    ...
    @seq.should_receive(:pointer).and_return("#{iksnr}/1/#{ikscd}")
  ...
  def test_checkout
    ...
    part2.should_receive(:checkout).times(1).and_return { assert true }
    part2.should_receive(:odba_delete).times(1).and_return { assert true }
    @package.parts.push part1, part2
    slentry = flexmock "sl entry"
    slentry.should_receive(:checkout).times(1).and_return { assert true }
  ...
  def test_ddd_dafalgan_kinder
    ...
    create_test_package(iksnr: 51231, ikscd: 0o31, price_public: 1.40,
  ...
  def test_delete_sl_entry
    ...
    reg.should_receive(:generic_type=).with(nil).and_return { assert true }
    reg.should_receive(:odba_isolated_store).and_return { assert true }
  ...
  def test_ikscd_writer
    @package.out_of_trade = true
    part = flexmock "part"
    part.should_receive(:fix_pointers).times(1).and_return { assert true }
    @package.parts.push part
    pacs = {"012" => @package}
    ...
    slentry.should_receive(:odba_store).times(1).and_return { assert true }
    @package.instance_variable_set :@sl_entry, slentry
    @package.ikscd = "015"
