#!/usr/bin/env ruby
# TestCompanyListState -- oddb -- 13.10.2003 -- maege@ywesee.com
...
module ODDB
	class TestCompanyListState < CompanyListState
...
class TestCompanyListState < Test::Unit::TestCase
	...
	def setup
		...
		@session.user = ODDB::RootUser.new
	end
	def test_intervals
		@state = ODDB::CompanyListState.new(@session, @company)
	...
	def test_sent_model
		@session.user_input = { :range	=>	'u-z' } 
		@state = ODDB::TestCompanyListState.new(@session, @company)
