#!/usr/bin/env ruby
# TestFachinfoConfirmState -- oddb -- 03.10.2003 -- rwaltert@ywesee.com
...
module ODDB
	class FachinfoConfirmState < GlobalState
...
class TestFachinfoConfirmState < Test::Unit::TestCase
	...
	def setup
		...
		@app = StubApp.new
		@session.app = @app
		@state = ODDB::FachinfoConfirmState.new(@session, []) 
	end
	...
	def test_update2
		...
		@session.user_equiv = true
		reg_pointer = ODDB::Persistence::Pointer.new([:registration, '12345'])
		reg = StubRegistration.new
		reg.iksnr = '12345'
		reg.pointer = reg_pointer
		@app.registration = reg
		fi_pointer = ODDB::Persistence::Pointer.new(:fachinfo)
