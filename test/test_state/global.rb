  ...
  module State
    ...
    class TestGlobal < Minitest::Test
      ...
      def test_aa_resolve_changelog_item
        setup_registration(54316, "/de/gcc/show/fachinfo/54316/diff/#{@@today}")
        skip("Niklaus did not have time to debug this assert")
      ...
      def test_resolve__self
        ...
        @state.instance_eval('@request_path = "request_path"', __FILE__, __LINE__)
        assert_equal(@state, @state.resolve)
      end
      ...
      def test_snapback_event__direct_event
        ...
        assert_nil(@state.instance_eval("DIRECT_EVENT", __FILE__, __LINE__))
      end

      def test_sort
        @state.instance_eval("@model = []", __FILE__, __LINE__)
      ...
      def test_zone_navigation
        assert_equal(@state.instance_eval("ZONE_NAVIGATION", __FILE__, __LINE__), @state.zone_navigation)
      end

      def test_compare_entries
        @state.instance_eval("@sortby = [:xxx, :sort]", __FILE__, __LINE__)
        skip("Niklaus did not have time to debug this assert")
        assert_equal(-1, @state.instance_eval('compare_entries("a","b")', __FILE__, __LINE__))
      end

      def test_compare_entries__0
        @state.instance_eval("@sortby = [:chomp!]", __FILE__, __LINE__)
        assert_equal(0, @state.instance_eval('compare_entries("a","b")', __FILE__, __LINE__))
      end
      ...
      def test_get_sortby!
        ...
        assert_equal([:sortvalue], @state.instance_eval("get_sortby!", __FILE__, __LINE__))
      end
