...
module ODDB
  ...
  class TestResilientLoop < Minitest::Test
    ...
    def test_full_loop_no_problem
      ...
      loop_entries.each{
        |entry|
          @r_loop.try_run(entry, timeout_in_secs = TimeoutValue) { result << entry; }
      }
      @r_loop.finished
    ...
    def test_full_loop_always_timeout
      ...
      begin
        loop_entries.each{
          |entry|
            @r_loop.try_run(entry, TimeoutValue) { sleep(TimeoutValue*2); result << entry; }
        }
        @r_loop.finished
    ...
    def test_full_loop_recovers_from_first_timeout
      ...
      begin
        loop_entries.each{
          |entry|
            @r_loop.try_run(entry, TimeoutValue) {  toggle = !toggle; sleep(TimeoutValue*2) if toggle; result << entry; }
        }
      rescue Timeout::Error
      end
      assert_equal(loop_entries, result)
