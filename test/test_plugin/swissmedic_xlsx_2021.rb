...
module ODDB
  ...
  class SwissmedicPluginTestXLSX2021 < Minitest::Test
    ...
    def setup
      ...
      FileUtils.mkdir_p(File.join(ODDB::WORK_DIR, 'xls'))
      FileUtils.mkdir_p(File.join(ODDB::WORK_DIR, 'xlsx'))
      @latest = File.join ODDB::TEST_DATA_DIR, 'xlsx', 'Packungen-latest.xlsx'
      @older  = File.join(ODDB::TEST_DATA_DIR, 'xlsx', 'Packungen-2019.01.31.xlsx')
      @plugin = flexmock('plugin', SwissmedicPlugin.new(@app, ODDB::WORK_DIR))
      ...
      @target   = File.join ODDB::WORK_DIR, 'xls',  @@today.strftime('Packungen-%Y.%m.%d.xlsx')
      ...
      FileUtils.cp(@prep_from, File.join(ODDB::WORK_DIR, 'xlsx','Erweiterte_Arzneimittelliste_HAM_31012019.xlsx'),
    ...
    def test_april_2021
      ...
      result = @plugin.update({:check => true})
      puts @plugin.report
