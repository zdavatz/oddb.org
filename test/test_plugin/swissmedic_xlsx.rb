...
module ODDB
  class SwissmedicPluginTestXLSX < Minitest::Test
    def setup
      ...
      @plugin.should_receive(:fetch_with_http).with("https://www.swissmedic.ch/swissmedic/de/home/services/listen_neu.html").and_return(File.read(@bag_listen)).by_default
      @plugin.should_receive(:open).with(ODDB::SwissmedicPlugin.get_packages_url).and_return(File.read(@current)).by_default
      ...
      @plugin.should_receive(:fetch_with_http).with(ODDB::SwissmedicPlugin.get_preparations_url).and_return(File.read(@prep_from)).by_default
    ...
    def check_agents(sequences)
      sequences.each { |seq|
        seq.compositions.each { |comp|
          comp.active_agents.each { |agent|
          ...
          comp.inactive_agents.each { |agent|
    ...
    def test_cleanup_active_agents_all_nil
      ...
      @plugin.cleanup_active_agents_with_nil
    end
  end
end
