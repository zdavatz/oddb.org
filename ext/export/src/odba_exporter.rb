...
module ODDB
  module OdbaExporter
    ...
    def self.compress_many(dir, name, files)
      FileUtils.mkdir_p(dir)
      Dir.chdir(dir)
      gz_name = name + ".tar.gz"
    ...
    def self.export_competition_xls(comp_id, dir, name, db_path = nil)
    ...
    def self.export_doc_csv(odba_ids, dir, name)
      safe_export(dir, name) { |fh|
        fh << <<~HEAD
    ...
    def self.export_generics_xls(dir, name)
      safe_export(dir, name) { |fh|
    ...
    def self.export_ean13_idx_th_csv(odba_ids, dir, name)
      safe_export(dir, name) { |fh|
        fh << <<~HEAD
    ...
    def self.export_idx_th_csv(odba_ids, dir, name)
      safe_export(dir, name) { |fh|
        fh << <<~HEAD
    ...
    def self.export_migel_csv(odba_ids, dir, name)
      safe_export(dir, name) { |fh|
        fh << <<~HEAD
    ...
    def self.export_narcotics_csv(odba_ids, dir, name)
      safe_export(dir, name) { |fh|
        odba_ids.each { |odba_id|
          item = ODBA.cache.fetch(odba_id, nil)
    ...
    def self.export_patent_xls(odba_ids, dir, name)
      nil_data = []
      safe_export(dir, name) { |fh|
    ...
    def self.export_price_history_csv(odba_ids, dir, name)
      epoch = Date.new 1979
      safe_export(dir, name) { |fh|
        dates = {}
        ...
        CSV.open(fh.path, "w", col_sep: ";") { |csv| csv << head }
        packages.sort_by { |pack| pack.name }.each do |pack|
    ...
    def self.remote_safe_export(dir, name, &block)
      FileUtils.mkdir_p(dir)
      Tempfile.open(name, dir) { |fh|
        ...
        FileUtils.chmod(0o644, newpath)
        compress(dir, name)
      }
      name
    end

    def self.safe_export(dir, name, &block)
      FileUtils.mkdir_p(dir)
      Tempfile.open(name, dir) { |fh|
        block.call(fh)
        fh.close
