...
@long_stat = {}
@empty_stat = {}
def self.analyse_one_patinfo(meta)
  image_subfolder = "/tmp/tmp_images"
  res = ODDB::FiParse.parse_patinfo_html(meta.cache_file, image_folder: image_subfolder)
  if meta.title
  ...
  File.open(dump_name, 'w+') do |f| f.puts res end
  ODDB::PatinfoDocument::CHAPTERS.each do |chapter|
...
def self.show_stat
  ...
  puts
  puts "Showing statistics for #{@nrTextInfos} analysied patinfos"
  puts
  puts "  Showing occurrences a given chapter was not found"
  pp @long_stat

  puts
  puts "  Showing nr occurrences and first 3 PI where chapter was empty"
...
HtmlAnalyse.analyse_all_patinfos
