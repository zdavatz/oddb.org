...
#      open(@path) { |fh| @fachinfo = @writer.extract(Hpricot(fh)) }
  ...
  class TestFachinfo_62580_Novartis_Seebri< Test::Unit::TestCase
    ...
    def test_name2
      assert_equal("Zusammensetzung\nWirkstoff\nGlycopyrronium als Glycopyrroniumbromid .\nHilfsstoffe\nLactose-Monohydrat ., M agnesiumstearat .\nColor: Gelborange S (E 110) , excipiens pro capsula .", @fachinfo.composition.to_s)
    end
...
Klare, farblos bis gelbliche Lösung von bitterem Geschmack.", @fachinfo.galenic_form.to_s)
      end
...
Eine Kautablette enthält 100 mg (mit Bruchrille) oder 25 mg Raltegravir als Raltegravir-Kalium.",  
      ...
      def test_composition_isentres
        assert_equal("Wirkstoff: Raltegravir
Hilfsstoffe:
Filmtablette:
...
", 
                     @fachinfo.composition.to_s)
        assert_equal("Zusammensetzung Wirkstoff: Raltegravir.\nHilfsstoffe\nFilmtablette:\nKern: mikrokristalline Cellulose, Lactose-Monohydrat, wasserfreies Calciumhydrogenphosphat, Hypromellose 2208, Poloxamer 407 (enthält 0,01% butyliertes Hydroxytoluol als Antioxidationsmittel, E 321), Natriumstearylfumarat, Magnesiumstearat. Filmüberzug: Polyvinylalkohol, Titandioxid, Polyethylenglykol 3350, Talkum, rotes Eisenoxid und schwarzes Eisenoxid.",
                     @fachinfo.composition.to_s)
      end                                                                                                                                                                                                                                                    
      def test_iksnrs
