  ...
  module Text
    ...
    class Table
      ...
      def column_widths # as pre-formatted paragraph
        @rows.each_with_object([]) { |row, memo|
    ...
    class Section
      ...
      def gsub! *args, &block
        @paragraphs.each { |paragraph| paragraph.gsub!(*args, &block) }
        @subheading.gsub!(*args, &block)
      end

      def to_s opts = {}
        lines = [@subheading] + @paragraphs.collect { |par| par.to_s opts }
    ...
    class Chapter
      ...
      def gsub! *args, &block
        @sections.each { |section| section.gsub!(*args, &block) }
