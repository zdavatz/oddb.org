      ...
      @atc_sorted or begin 
        if(overflow?)
          ...
          when :interaction, :unwanted_effect
            ...
            @atc_sorted = @atc_facades.sort_by do |atc_class|
    ...
    def apply_filters(session=nil)
      ...
      @atc_facades.each do |facade|
        @package_filters.each do |key, filter_proc|
          facade.packages = facade.packages.select do |pack| filter_proc.call pack end
      ...
      @atc_facades.each do |facade|
        @package_filters.each do |key, filter_proc|
          ...
          puts "search_result #{__LINE__}: #{key} Before we have #{packages.size} packages. @package_count #{@package_count.inspect}/#{package_count}"
          @packages << packages.select do |pack| filter_proc.call pack end
          puts "search_result #{__LINE__}: #{key}: After we have  #{@packages.size} packages. @package_count #{@package_count.inspect}/#{package_count}"
        end
      end if false
      @packages.flatten!
      @package_count = nil
