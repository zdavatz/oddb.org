...
module ODDB
  ...
  class LookandfeelAnthroposophy < SBSM::LookandfeelWrapper
    SEQUENCE_FILTER = proc { |seq| seq.complementary_type == :anthroposophy }
    HTML_ATTRIBUTES = {
      logo: {
        "width"	=>	"370",
        "height"	=>	"166"
  ...
  class LookandfeelHomeopathy < SBSM::LookandfeelWrapper
    SEQUENCE_FILTER = proc { |seq| seq.complementary_type == :homeopathy }
    HTML_ATTRIBUTES = {
      logo: {
        "width"	=>	"370",
        "height"	=>	"166"
  ...
  class LookandfeelPhytoPharma < SBSM::LookandfeelWrapper
    SEQUENCE_FILTER = proc { |seq| seq.complementary_type == :phytotherapy }
    HTML_ATTRIBUTES = {
      logo: {
        "width"	=>	"344",
        "height"	=>	"106"
  ...
  class LookandfeelDesitin < SBSM::LookandfeelWrapper
    SEQUENCE_FILTER = proc { |seq| (comp = seq.company) && comp.oid == 215 }
