...
module ODDB
  class Updater
    ...
    def update_bsv
      ...
      wrap_update(klass, subj) {
        ...
        LogFile.append("oddb/debug", " return_value_BsvXmlPlugin.update = " + return_value_plug_update.inspect, Time.now)
  ...
  return_value_plug_update
    ...
    def log_notify_bsv(plug, date, subj, pointer)
      ...
      LogFile.append("oddb/debug", " return_value_log_notify = " + return_value_log_notify.inspect, Time.now)
      ...
      LogFile.append("oddb/debug", " return_value_log2_notify = " + return_value_log2_notify.inspect, Time.now)
      return_value_log2_notify
    end

    def notify_error(klass, subj, error)
      log = Log.new(@@today)
      klass.get_memory_error if klass.respond_to?(:get_memory_error)
      msg ||= " "
      log.report = [
        "Plugin: #{klass}",
        msg,
    ...
    def update_immediate(klass, subj, update_method = :update)
      ...
      log.update_values(log_info(plug))
      log.notify(subj)
    rescue => e # RuntimeError, StandardError => e
      notify_error(klass, subj, e)
    end
