...
=end
  ...
  def search_registration_holder(key)
    ...
    companies = retrieve_from_index("company_index", key)
    matching_companies = companies.find_all{|item| item.business_area.to_s == ODDB::BA_type::BA_pharma.to_s }
    matching_companies
  end
...
module ODDB
    ...
    MEMORY_LIMIT_CRAWLER = 6000  # ruby 1.9.3 worked with 1450 # 1,4 GB. Ruby 2.4.0 show (via htop) useages between 1200 and 1700 MB
		RUN_CLEANER = true
		RUN_UPDATER = false
		SESSION = Session
