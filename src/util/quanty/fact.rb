...
class Quanty
  class Fact < Hash
    ...
    def decomp(a)
      unit = if /^([µA-Za-z_]+([A-Za-z_0-9-]+[A-Za-z_])?)$|^[$%'"]'?$/ou.match?(a)
        # if /^[A-Za-z_0-9$%-]+$/o =~ a
        List[a] || find_prefix(a, 0) ||
    ...
    def pow!(other)
      raise unless other.is_a?(Numeric)
