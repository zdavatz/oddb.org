...
module ODDB
  module Util
    ...
    def self.check_column_indices(sheet)
      sheet = sheet.rows.slurp
      if /Zugelassene Verpackungen/i.match?(sheet[2][0])
        row = sheet[3] # Headers are found at row 3
      elsif /Zulassungs-nummer/i.match?(sheet[4][0])
      ...
      COLUMNS_FEBRUARY_2019.each { |key, value|
        header_name = row[COLUMNS_FEBRUARY_2019.keys.index(key)]
        unless value.match(header_name)
          puts "#{__LINE__}: #{key} ->  #{COLUMNS_FEBRUARY_2019.keys.index(key)} #{value}\nbut was  #{header_name}" if $VERBOSE
          error_2015 = "Packungen.xlslx_has_unexpected_column_#{COLUMNS_FEBRUARY_2019.keys.index(key)}_#{key}_#{value}_but_was_#{header_name}"
          break
        end
      }
