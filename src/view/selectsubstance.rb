#!/usr/bin/env ruby
# SelectSubstanceView -- oddb -- 28.04.2003 -- benfay@ywesee.com
...
module ODDB
	...
	class SelectSubstanceComposite < HtmlGrid::Composite
		...
		def assigned_list(model, session)
			AssignedList.new(model.assigned, session, self)	
		end
	end	
	class SelectSubstanceView < PrivateTemplate
		CONTENT = SelectSubstanceComposite
