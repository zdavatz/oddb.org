      ...
      class FeedbackList < View::FeedbackList
        ...
        def name(model, session = @session)
          ...
          if model.name.respond_to?(:encode)
            model.name.encode("UTF-16BE", invalid: :replace, undef: :replace, replace: "?").encode("UTF-8")
          else
            ""
          end
        end
        ...
        def message(model, session = @session)
          ...
          model.message.encode("UTF-16BE", invalid: :replace, undef: :replace, replace: "?").encode("UTF-8")
        end
      end
