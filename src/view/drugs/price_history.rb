  ...
  module View
    module Drugs
      class PriceHistoryList < HtmlGrid::List
        ...
        def authorities(model)
          collect_data(model, :authority).collect do |key|
            ...
            span.value = @lookandfeel.lookup(:"price_authority_#{key}") { key }
            span.set_attribute("title", @lookandfeel.lookup(:"price_authority_title_#{key}") { key })
            span
          end
        end
        ...
        def mutation_codes(model)
          collect_data(model, :mutation_code).collect do |key|
            @lookandfeel.lookup(:"sl_mutation_#{key}") { key }
          end.join(", ")
        end

        def origins(model)
          collect_data(model, :origin).each_with_object([]) do |url_and_date, memo|
            ...
            link.href = link.value = url if /http/.match?(url)
            memo.push link
            if date
              memo.push " ", date
            end
      ...
      class PriceHistory < PrivateTemplate
        ...
        def init
          @model.pointer_descr = if pack = @model.package
            @lookandfeel.lookup(:price_history_for, pack.name)
          else
            @lookandfeel.lookup(:price_history)
          end
          super
        end
      end
