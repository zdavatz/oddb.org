    ...
    module Drugs
      class PackageInnerComposite < HtmlGrid::Composite
        ...
        def init
          ...
          if @lookandfeel.enabled?(:feedback) and !@lookandfeel.disabled?(:feedback)
            components.update(
              [0, 11] => :feedback_label,
              [1, 11] => :feedback,
              [0, 12] => :pharmacode
        ...
        def name(model, session = @session)
          ...
          link.href = if @lookandfeel.disabled?(:best_result)
      ...
      class PackageComposite < HtmlGrid::Composite
        ...
        def to_html(context)
          ...
          html = super
          @lookandfeel.enabled?(:facebook_share, true) ? facebook_sdk + html : html
        end
      end
