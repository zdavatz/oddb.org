#!/usr/bin/env ruby
# View::Drugs::IncompleteSequence -- oddb -- 20.06.2003 -- hwyss@ywesee.com 
...
class IncompleteSequenceInnerComposite < HtmlGrid::Composite
	...
	def active_sequence(model, session)
		if((reg = @session.app.registration(model.iksnr)) \
			...
			View::Drugs::RootSequenceComposite.new(seq, session, self)
		end
	end
end
class IncompleteSequenceForm < View::Drugs::SequenceForm
	EVENT = :update_incomplete
end
class IncompleteSequenceComposite < View::Drugs::RootSequenceComposite
	COMPONENTS = {
		[0,0]	=>	:sequence_name,
		[0,1]	=>	IncompleteSequenceForm,
		[0,2]	=>	:sequence_agents,
		[0,3]	=>	:sequence_packages,
		[0,4]	=>	View::Drugs::IncompleteSequenceInnerComposite,
	}
	CSS_MAP = {
		[0,0]	=>	'th',
		[0,4]	=>	'composite',
...
class IncompleteSequence < View::PrivateTemplate
	...
	CONTENT = View::Drugs::IncompleteSequenceComposite
end
		end
	end
end
