#!/usr/bin/env ruby
# View::Drugs::IncompletePackage -- oddb -- 23.06.2003 -- hwyss@ywesee.com 
...
class IncompletePackageInnerComposite < HtmlGrid::Composite
	...
	def active_package(model, session)
		if((reg = @session.app.registration(model.iksnr)) \
			...
			View::Drugs::PackageComposite.new(pack, session, self)
		end
	end
...
class IncompletePackageForm < View::Drugs::PackageForm
...
class IncompletePackageComposite < View::Drugs::PackageComposite
	COMPONENTS = {
		[0,0]	=>	:package_name,
		[0,1]	=>	View::Drugs::IncompletePackageForm,
		[0,2]	=>	View::Drugs::IncompletePackageInnerComposite,
	}
end
class IncompletePackage < View::PrivateTemplate
	CONTENT = View::Drugs::IncompletePackageComposite
