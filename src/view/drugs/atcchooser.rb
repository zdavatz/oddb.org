...
# ODDB::View::Drugs::AtcChooser -- oddb.org -- 09.11.2012 -- yasaka@ywesee.com
      ...
      module AtcLink
        ...
        def atc_drugbank_link(atc, session = @session)
          if atc.db_id
            ...
            link.href = if atc.db_id.is_a? Array # link to search result
              "http://www.drugbank.ca/search?utf8=✓&query=#{atc.code}&commit=Search"
            else
              "http://www.drugbank.ca/drugs/#{atc.db_id}"
            end
            link
          end
        end
