#!/usr/bin/env ruby
# ResultList -- oddb -- 03.03.2003 -- aschrafl@ywesee.com
	...
	class AtcHeader < HtmlGrid::Composite
		...
		def pages(model, session)
			...
			if(state.respond_to?(:pages) \
				&& (pages = state.pages) \
				&& pages.size > 1)
				pager = PagerView.new(pages, session, self)
			end
			pager
		end
	end
	class ResultList < HtmlGrid::List
		...
		def company_name(model, session)
			...
			if(@lookandfeel.enabled?(:powerlink, false) && comp.powerlink)
				...
				ODDB::PointerLink.new(:name, model.company, session, self)
			else
