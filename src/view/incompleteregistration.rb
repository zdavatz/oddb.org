#!/usr/bin/env ruby
# IncompleteRegistrationView -- oddb -- 19.06.2003 -- hwyss@ywesee.com 
...
module ODDB
	class IncompleteRegistrationForm < RegistrationForm
	...
	class IncompleteRegistrationInnerComposite < HtmlGrid::Composite
		...
		def active_registration(model, session)
			if(registration = session.app.registration(@model.iksnr))
				RootRegistrationComposite.new(registration, session, self)
			end
		end
	end
	class IncompleteRegistrationComposite < RootRegistrationComposite
		COMPONENTS = {
			[0,1]	=>	IncompleteRegistrationForm,
			[0,2]	=>	:registration_sequences,
			[0,3]	=>	IncompleteRegistrationInnerComposite,
		}
		CSS_MAP = {
			[0,0]	=>	'th',
			[0,3]	=>	'composite',
	...
	class IncompleteRegistrationView < PrivateTemplate
		CONTENT = IncompleteRegistrationComposite
