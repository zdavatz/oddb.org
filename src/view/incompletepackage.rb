#!/usr/bin/env ruby
# IncompletePackageView -- oddb -- 23.06.2003 -- hwyss@ywesee.com 
...
module ODDB
	class IncompletePackageInnerComposite < HtmlGrid::Composite
		...
		def active_package(model, session)
			if((reg = @session.app.registration(model.iksnr)) \
				...
				PackageComposite.new(pack, session, self)
			end
		end
	...
	class IncompletePackageForm < PackageForm
	...
	class IncompletePackageComposite < PackageComposite
		COMPONENTS = {
			[0,0]	=>	:package_name,
			[0,1]	=>	IncompletePackageForm,
			[0,2]	=>	IncompletePackageInnerComposite,
		}
	end
	class IncompletePackageView < PrivateTemplate
		CONTENT = IncompletePackageComposite
