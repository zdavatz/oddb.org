    ...
    module Migel
      ...
      class SearchedList < HtmlGrid::List
        ...
        def article_name(model = @model, session = @session)
          name = if model.article_name.respond_to?(session.language)
        ...
        def companyname(model = @model, session = @session)
          name = if model.companyname.respond_to?(session.language)
            CGI.unescape(model.companyname.send(session.language))
          else
            CGI.unescape(model.companyname)
          end
          name.to_s.encode("utf-8") if name
        end

        def size(model = @model, session = @session)
          size = if model.size.respond_to?(session.language)
        ...
        def sort_link(header_key, matrix, component)
          ...
          if @session.user_input(:search_query)
            ...
            args = [:migel_code, @model.first.migel_code.encode("utf-8").delete("."), sort_way, component.to_s]
            if page
              args.concat [:page, page + 1]
            end
