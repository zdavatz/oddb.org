      ...
      class ProductInnerComposite < HtmlGrid::Composite
        ...
        def description(model, key = :migel_product)
          ...
          if str
            ...
            value.value = str.gsub(@@migel_pattern) {
              if group_cd = $~[1]
                args = if subgroup_cd = $~[2]
                  if product_cd = $~[3]
                    [:migel_product, group_cd + subgroup_cd + product_cd.delete(".")]
                  else
                    [:migel_subgroup, group_cd + subgroup_cd]
                  end
                else
                  [:migel_group, group_cd]
                end
              end
        ...
        def migel_code(model)
          ...
          if items = model.items and !items.empty?
            link = PointerLink.new(:to_s, model, @session, self)
            link.value = code
            link.href = @lookandfeel._event_url(:migel_search, {migel_code: code.delete(".")})
            link
          else
