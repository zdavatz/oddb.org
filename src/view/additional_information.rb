...
module ODDB
    ...
    module PartSize
      ...
      def google_search(model, session=@session)
        text = model.localized_name(@session.language).to_s.encode('utf-8')
      ...
      def minifi(model, session=@session)
        ...
        if model.is_a?(ODDB::Package)
          ...
          name       = model.article_name.to_s.encode('utf-8')
          migel_code = model.migel_code.to_s.encode('utf-8')
      ...
      def qty_unit(model, session=@session)
        if(model.qty || model.unit)
          ...
          qty = model.qty.to_s.encode('utf-8')
      ...
      def twitter_share(model, session=@session)
        ...
        if model.is_a?(DRb::DRbObject)
          ...
          base = model.localized_name(session.language).to_s.encode('utf-8')
          code = model.pharmacode.to_s.encode('utf-8')
