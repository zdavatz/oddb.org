  ...
  module View
    ...
    module AdditionalInformation
      ...
      def ikscat(model, session = @session)
        ...
        @lookandfeel._event_url(:ajax_swissmedic_cat, [:reg, model.iksnr, :seq, model.seqnr, :pack, model.ikscd])
        txt.css_id = "ikscat_#{@ikscat_count}"
        [drugshortage(model, @session), txt].compact
      end
      ...
      def limitation_link(sltxt, model = nil)
        ...
        link.href = if reg and seq and pack
          @lookandfeel._event_url(:limitation_text, [:reg, reg, :seq, seq, :pack, pack])
        else
          @lookandfeel._event_url(:resolve, {"pointer" => CGI.escape(sltxt.pointer.to_s)})
        end
        link.set_attribute("title",
      ...
      def narcotic(model, session = @session)
        if model.bm_flag
          if @lookandfeel.disabled?(:explain_narc_url)
            HtmlGrid::SimpleLabel.new(:square_narc, model, @session, self)
          else
      ...
      def patinfo(model, session = @session)
        if model.respond_to?(:has_patinfo?) and model.has_patinfo?
          ...
          if pdf_patinfo = model.pdf_patinfo and !pdf_patinfo.empty?
            ...
            klass = HtmlGrid::Link
            smart_link_format = if model.is_a?(Package)
              [:reg, model.iksnr, :seq, seqnr, :pack, model.ikscd]
            else
              [:reg, model.iksnr, :seq, seqnr]
            end
