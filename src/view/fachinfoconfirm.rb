#!/usr/bin/env ruby
# FachinfoConfirmView -- oddb -- 26.09.2003 -- rwaltert@ywesee.com
...
module ODDB
	...
	class FachinfoConfirmForm < FormList
		...
		def language_select(model, session)
			...
			select = FachinfoLanguageSelect.new(name, model, session, self)
	...
	class FachinfoConfirmView < PrivateTemplate
		CONTENT = FachinfoConfirmComposite
	end
end
