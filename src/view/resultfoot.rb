  ...
  module View
    class ExplainResult < HtmlGrid::Composite
      ...
      def explain_square_link(model, key, url = nil)
        exp_key = "explain_#{key}".to_sym
        url_key = "explain_#{key}_url".to_sym
        square_key = "square_#{key}".to_sym
        value = @lookandfeel.lookup(exp_key) ? @lookandfeel.lookup(exp_key) : @lookandfeel.lookup(exp_key.to_s)
        unless @lookandfeel.disabled?(url_key.to_sym)
