      ...
      class DownloadItemForm < Form
        ...
        def hidden_fields(context)
          ...
          if !@session.error? and file = @session.user_input(:buy)
            ...
            hidden << if (month = @session.user_input(:month))
              context.hidden("months[#{file}]", month)
            else
              context.hidden("months[#{file}]", default_month(file))
            end
          end
          hidden
        end
      ...
      class DownloadItemComposite < View::User::DownloadExportInnerComposite
        ...
        def example_item(model)
          if file = @session.user_input(:buy)
            example_method = ("example_" + file.tr(".", "_").downcase).intern
            if respond_to?(example_method)
              send(example_method, model, @session)
            end
          end
        end
        ...
        def datadesc_item(model)
          if file = @session.user_input(:buy)
            desc_method = ("datadesc_" + file.tr(".", "_").downcase).intern
            if respond_to?(desc_method)
              send(desc_method, model, @session)
            elsif File.exist?(
