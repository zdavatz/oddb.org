...
#    [7,10] => :example_narcotics_yaml,
    ...
    [2,30] => :radio_oddbdat_1,
    [3,30] => :radio_oddbdat_12,
    [5,30] => :datadesc_oddbdat,
  ...
  def example_oddb_dat_zip(model, session)
    example('oddb.dat.zip')
  end
  def example_oddb_with_migel_dat_zip(model, session)
  ...
  def oddb_dat_zip_export(model, session)
    checkbox_with_filesize("oddb.dat")
  end
  def oddb_dat_zip_price(model, session)
    once('oddb.dat')
  end
  def oddb_with_migel_dat_zip_export(model, session)
    checkbox_with_filesize("oddb_with_migel.dat")
  end
  def oddb_with_migel_dat_zip_price(model, session)
