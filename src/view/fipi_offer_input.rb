#!/usr/bin/env ruby
# FiPiOfferInputView -- oddb -- 28.06.2004 -- maege@ywesee.com
...
module ODDB
	class FiPiRadioButtons < HtmlGrid::List
		...
		def radio_text(model, session)
			case model.name[0,2]
			when 'fi'
				prize =  ODDB::FiPiOfferInputState::FiPiOffer::FI_UPDATE
			when 'pi'
				prize =  ODDB::FiPiOfferInputState::FiPiOffer::PI_UPDATE
			end
	...
	class FiPiOfferInputForm < Form 
		...
		def fi_update(model, session)
			...
			FiPiRadioButtons.new([ radio1, radio2 ], session)	
		end
		...
		def fipi_activation_charge_value(model, session)
			prize =  ODDB::FiPiOfferInputState::FiPiOffer::FIPI_ACTIVATION_CHARGE
	...
	class FiPiOfferInputView < PublicTemplate
		CONTENT = FiPiOfferInputComposite
		SNAPBACK_EVENT = :home
	end
end
