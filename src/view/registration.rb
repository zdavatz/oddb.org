#!/usr/bin/env ruby
# RegistrationView -- oddb -- 07.03.2003 -- hwyss@ywesee.com 
...
module ODDB
	...
	class RootRegistrationSequences < FormList
		include RegistrationSequenceList
	end
	...
	class RegistrationForm < Form
		...
		def init
			if(@session.user.is_a?(RootUser) && !@model.is_a?(Persistence::CreateItem))
	...
	class RegistrationComposite < HtmlGrid::Composite
		COMPONENTS = {
			[0,1]		=>	RegistrationInnerComposite,
		...
		DEFAULT_CLASS = HtmlGrid::Value
		SEQUENCES = RegistrationSequences
	...
	class RootRegistrationComposite < RegistrationComposite
		COMPONENTS = {
			[0,1]		=>	RegistrationForm,
			[0,2]		=>	:registration_sequences,
			[0,3]		=>	"th_source",
			[0,4]		=>	:source,
		}
		SEQUENCES = RootRegistrationSequences
		CSS_MAP = {
			[0,0]	=>	'th',
			[0,3]	=>	"subheading",
		}
	end
	class RegistrationView < PrivateTemplate
		CONTENT = RegistrationComposite
		SNAPBACK_EVENT = :result
	end
	class RootRegistrationView < RegistrationView
		CONTENT = RootRegistrationComposite
	end
end
