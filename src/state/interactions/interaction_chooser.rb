      ...
      class InteractionChooser < State::Interactions::Global
        ...
        def init
          ...
          if search_code
            ...
            items.each { |item|
              if item.is_a?(String) and item.length == 5 # it is an iksrn
                registration = @session.app.registration(item)
                pack = registration.packages.first
                next unless pack
                drugs[pack.barcode] = pack
              elsif item.is_a?(String) and item.length == 7 # it is an ATC code
              ...
              elsif item.is_a?(String) and item.length == 13 # it is an barcode/ean13
                next unless item
