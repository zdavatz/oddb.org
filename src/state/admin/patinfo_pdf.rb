      ...
      module PatinfoPdfMethods # for sequence, package
        ...
        def get_patinfo_input(input)
          ...
          if (html_file = @session.user_input(:html_upload)) \
             ...
             && !is_a?(ODDB::State::Admin::Package) # only sequence
            @model.pdf_patinfo = nil
            lang = @session.user_input(:language_select)
            ptr = if (patinfo = @model.patinfo)
              patinfo.pointer
            else
              Persistence::Pointer.new(:patinfo).creator
            end
            ...
            if !company.invoiceable?
              ...
              pi_file.rewind
              filename = if is_a?(ODDB::State::Admin::Package)
              ...
              input.store(:patinfo, nil) unless is_a?(ODDB::State::Admin::Package)
              unless is_a?(ODDB::State::Admin::Package)
        ...
        def store_slate_item(time, type)
          ...
          text = if is_a?(ODDB::State::Admin::Package)
