#!/usr/bin/env ruby
# State::Drugs::Package -- oddb -- 14.03.2003 -- hwyss@ywesee.com 
...
class Package < State::Drugs::Global
	...
	def new_item
		...
		State::Drugs::SlEntry.new(@session, item)
	end
	def update
...
class CompanyPackage < State::Drugs::Package
	def init
		super
		unless(allowed?)
			@default_view = View::Drugs::Package
		end
	end
	def delete
		if(allowed?)
