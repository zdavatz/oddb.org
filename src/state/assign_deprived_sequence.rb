#!/usr/bin/env ruby
#AssignDeprivedSequence -- oddb -- 15.12.2003 -- rwaltert@ywesee.com

require 'view/assign_deprived_sequence'

module ODDB
	class AssignDeprivedSequenceState < GlobalState
		VIEW = AssignDeprivedSequenceView
=begin
...
=end
		...
		def preview 
			if(lang = @session.user_input(:language_select))
				...
				PatinfoPreviewState.new(@session, doc)
			else
				self
			end
		end
