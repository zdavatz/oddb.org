...
require 'state/assign_deprived_sequence'
require 'util/interval'

module ODDB
	class PatinfoDeprivedSequencesState < GlobalState
		include Interval
		VIEW = PatinfoDeprivedSequencesView
		...
		def select_seq
			...
			if(error?)
				...
				AssignDeprivedSequenceState.new(@session, seq)
			end
		end
		def shadow_pattern
			begin
