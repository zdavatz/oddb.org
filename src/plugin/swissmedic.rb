...
module ODDB
  class SwissmedicPlugin < Plugin
    ...
    def trace_msg(msg)
      return
      $stdout.puts "#{Time.now} #{File.basename(caller(1..1).first)}: #{msg}"
    end
    ...
    def mustcheck(iksnr, opts = {})
      ...
      if opts[:iksnrs].nil? or opts[:iksnrs].index(iksnr)
      ...
      if file2open and File.exist?(file2open)
        ...
        if @known_packages.size > 0
          @app.registrations.each do |iksnr, reg|
            ...
            reg.active_packages.each do |pack|
    ...
    def trace_memory_useage # Good default is 16 GB, afterwards the server slows down a lot
    ...
    def check_all_packages(file2open)
      ...
      worksheet.rows.each do |row|
        ...
        new_date = if /unbegrenzt/i.match?(cell(row, @target_keys.keys.index(:expiry_date)).to_s)
          # LogFile.debu "Setting unbegrenzt for #{iksnr} #{seqnr} #{ikscd}"
          nil # Date.new(2099,12,31).strftime(DATE_FORMAT)
        else
          Date.parse(cell(row, @target_keys.keys.index(:expiry_date))).strftime(DATE_FORMAT)
        end
    ...
    def update(opts = {}, file2open = get_latest_file("Packungen"))
      ...
      unless @@loaded
        begin
          @@loaded = true
          filename = File.join(__dir__, "parslet_compositions.rb")
      ...
      start_time = Time.new
      threads = []
      threads << Thread.new { trace_memory_useage }
    ...
    def known_data(latest)
      ...
      data.first.delete_if { |iksnr, row| @export_registrations[iksnr] }
      data
    end
    ...
    def mail_notifications
      ...
      if (grp = @app.log_group(:swissmedic)) && (log = grp.latest)
        ...
        companies = all_flags.each_with_object({}) { |(pointer, flgs), memo|
    ...
    def report
      atcless = @app.atcless_sequences.collect { |sequence|
        defined?(resolve_link) ? resolve_link(sequence.pointer) : "Unable to resolve_link sequence: #{sequence}"
      }.sort
      lines = [
        "ODDB::SwissmedicPlugin - Report #{@@today.strftime(DATE_FORMAT)}",
        "Total time to update: #{"%.2f" % @update_time} [m]"
      ]
      lines += if @update_comps
        [
    ...
    def update_active_agent(seq, composition, parsed_substance)
      ...
      ptr = if active
        if (agent = composition.active_agent(parsed_substance.name))
          from = "active_agent"
      ...
      elsif (agent = composition.inactive_agent(parsed_substance.name))
        from = "inactive_agent"
    ...
    def update_compositions(sequence, row, opts = {create_only: false}, composition_text, parsed_comps)
      ...
      if !@update_comps && opts[:create_only] && !sequence.active_agents.empty?
        ...
        names.collect { |name| update_substance(name) }
        ...
        parsed_comps.each_with_index do |parsed_comp, comp_idx|
          ...
          parsed_comp.substances.each_with_index { |substance, parsed_idx|
            ...
            next if defined?(Minitest) and !composition_in_db # for unknown reasons we we cannot create the pointer when running under Minitest
            args = {
            ...
            if substance.is_active_agent
              ...
              inactive_agents.push updated_agent
            end
          ...
          if composition_in_db.nil?
            ...
            composition_in_db.odba_store
            sequence.odba_store
          elsif !(parsed_comps.size == 1 && composition_in_db.substances.empty?)
            composition_in_db.active_agents.dup.each_with_index { |act, act_idx|
              if act and act.substance
    ...
    def update_galenic_form(seq, comp, row, opts = {})
      ...
      if (german = seq.name_descr) && !german.empty?
        ...
        LogFile.debug " update_galenic_form don't know how to update the galenic form. #{seq.name_descr.inspect} or source #{comp.source}"
      end
    end
    ...
    def update_excipiens_in_composition(seq, parsed_compositions)
      ...
      seq.compositions.each_with_index { |db_composition, idx|
    ...
    def sanity_check_deletions(diff)
      table = diff.registration_deletions.each_with_object({}) { |(iksnr, _), memo|
        ...
        sequence.compositions.each do |composition|
          ...
          if wrong_active_agents.size > 0
            ...
            wrong_active_agents.each do |wrong_agent|
              ...
              LogFile.debug "Deleting wrong_active_agent odba_store #{wrong_agent.pointer} is #{wrong_agent.oid} #{wrong_agent}"
          ...
          if wrong_inactive_agents.size > 0
            ...
            wrong_inactive_agents.each do |wrong_agent|
              @@corrected << wrong_agent.pointer
              LogFile.debug "Deleting wrong_inactive_agent odba_store #{wrong_agent.pointer} is #{wrong_agent.oid} #{wrong_agent}"
