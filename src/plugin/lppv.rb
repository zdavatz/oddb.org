...
module ODDB
  class LppvPlugin < Plugin
      ...
      worksheet.rows.each do |row|
        ...
        if rows == 1
          ...
          COL.each do |key, value|
            ...
            raise "Unexpected column name #{actual_name} does not match exepect #{key}" unless actual_name.eql?(key.to_s)
          end
        else
          ...
          row[COL[:muteDate]].to_s
          row[COL[:muteTyp]].to_s
          # puts "read #{pharmacode} #{gtin} #{name}"
          if gtin.to_i > 0
            @eans << gtin
          elsif pharmacode.length > 0
    ...
    def update_package(package, data)
      ...
      if data.delete(package.barcode) || data.delete(package.pharmacode)
