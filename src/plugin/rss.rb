  ...
  class RssPlugin < Plugin
    ...
    def detail_info(host, container, count = false)
      ...
      if count
        ...
        if @app.rss_updates[@name] and date >= @app.rss_updates[@name].first
          @new_entry_count += 1
          LogFile.append("oddb/debug", " rss adding new entry: #{date} #{title}", Time.now.utc)
        end
      end
      entry
    end
      ...
      LookandfeelBase::DICTIONARIES.each_key do |lang|
        ...
        while true
          ...
          first_page.xpath(".//div[@class='row']").each do |item|
            ...
            entries[lang] << result if result and !entries[lang].index { |x| x[:title] == result[:title] and x[:date] == result[:date] }
          end
          step_nr += 1
          # In January 2019 swissmedic changed the text from "Keine Resultate" to "Keine Ergebnisse" and the import never finished
          LogFile.append("oddb/debug", " swissmedic_entries_of: #{type} #{base_uri.gsub("1", step_nr.to_s)}", Time.now.utc) if step_nr > 20
    ...
    def update_swissmedic_feed(type)
      ...
      unless entries.empty?
        ...
        # re-update (overwrite)
        @app.rss_updates[@name] = if @current_issue_count > 0
          [@@today, @current_issue_count]
        else
          previous_update
        end
        ...
        if @new_entry_count > 0
          @report = {
            "New #{type}.rss feeds" => @new_entry_count,
