...
module ODDB
  class Atc_lessPlugin < Plugin
    ...
    def update_atc_codes
      ...
      [@packungen_xlsx, @refdata_xml].each { |file|
      ...
      worksheet.rows.each do |row|
        ...
        atc_code_swissmedic = row[atc_col] || nil
        reg_seq = "#{iksnr}/#{seqnr}"
      ...
      @app.registrations.values.each { |registration|
    ...
    def report
      ...
      lines << if @missing_sequences.empty? # no expiration date
        "Swissmedic: All sequences present"
      else
        "Skipped #{@missing_sequences.size} sequences#{join_string}#{@missing_sequences.join(join_string)}"
      end

      lines << if @atc_code_was_nil.empty? # no expiration date
        "No empty ATC-codes found"
      else
        "Replaced #{@atc_code_was_nil.size} empty ATC-code in#{join_string}#{@atc_code_was_nil.join(join_string)}"
      end

      lines << if @atc_code_corrected.empty? # no expiration date
        "All found ATC-codes were correct"
      else
        "Corrected #{@atc_code_corrected.size} ATC-code in sequences#{join_string}#{@atc_code_corrected.join(join_string)}"
      end

      lines << if @no_such_atc_code_in_database.size == 0
        "All ATC codes present in database"
      else
        "#{@no_such_atc_code_in_database.size} ATC codes absent in database#{join_string}#{@no_such_atc_code_in_database.join(join_string)}"
      end

      lines << "Checked against #{@atc_codes_in_refdata.size} ATC-codes from RefData"

      lines << if @atc_code_longer_from_refdata.size == 0
        "All ATC codes from swissmedic are as long as those from refdata"
      else
        "#{@atc_code_longer_from_refdata.size} ATC code taken from refdata where they are longer#{join_string}#{@atc_code_longer_from_refdata.join(join_string)}"
      end

      lines << if @obsolete.size == 0
        "No obsolete sequence '00' found"
      else
        "Deleted #{@obsolete.size} sequences '00' in registrations#{join_string}#{@obsolete.join(join_string)}"
      end

      lines << if @missing_registrations.empty? # no expiration date
        "Swissmedic: All registrations present"
      else
        "Skipped #{@missing_registrations.size} registrations#{join_string}#{@missing_registrations.join(" ")}"
      end
