...
module ODDB
  class ShortagePlugin < Plugin
    ...
    def initialize app, opts={:reparse => false}
      ...
      FileUtils.rm_f(@latest, :verbose => true) if @options[:reparse] && File.exist?(@latest)
    end
    ...
    def report
      ...
      txt = sprintf(fmt, @found.size, @deleted.size, @changes.size, @duration_in_secs.to_i)
      txt << "\nGTIN of concerned packages is\n"
      txt << @changes.keys.join("\n")
      txt << "\nChanges were:\n"
      @changes.each {|gtin, changed| txt << "#{gtin} #{changed.join("\n              ")}"; txt << "\n" }
      txt << "\nDeleted were:\n"
      @deleted.each {|gtin| txt << "#{gtin}\n" }
      txt
    end
    def update(agent=Mechanize.new)
      ...
      @shortages.each do |shortage|
        ...
        added_info.shortage_url  = (BASE_URI + '/' + shortage.css('a').first.attributes.first.last.value).clone
      ...
      old_packages_with_shortage.each do |package|
        ...
        @deleted << "#{package.barcode};#{package.atc_class.code};#{package.name}"
      ...
      @found.each do |gtin, info|
        ...
        @changes["#{gtin};#{pack.atc_class.code};#{pack.name}"] = changed
