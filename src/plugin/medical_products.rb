...
module ODDB
  class MedicalProductPlugin < Plugin
    ...
    def update
      ...
      atc_code ||= add_dummy_medical_product
      @options[:files].each { |param|
        ...
        files.each { |file|
          ...
          packages.each { |packInfo|
            ...
            @app.registration(packInfo.iksnr)
            ...
            unless sequence.atc_class
              @app.update(sequence.pointer, {atc_class: atc_code.code}, :medical_product)
            end
